...
const AddUserModal = ({ onClose }) => {
    ...
    const [addUser, { loading: newUserLoading, error: newUserError, data: newUserData }] = useMutation(ADD_USER);
  ...
  useEffect(() => {
    if (schoolData && schoolData.schools && schoolData.schools.length > 0) {
      setSelectedSchoolId(schoolData.schools[0].id); // Set the ID of the first school
    }
    ...
    <table className="user-table">
        ...
        <tbody>
          ...
          <td>
            ...
            <td>
              <input type="text" value={yearInput}  onChange={(e) => setYearInput(e.target.value)}/>
            </td>
            <td>
      ...
      <div className="form-group">
        ...
        <textarea id="emails" placeholder="Add multiple users by separating the emails with a comma." value={emailInput}
                onChange={(e) => setEmailInput(e.target.value)}></textarea>
      </div>
